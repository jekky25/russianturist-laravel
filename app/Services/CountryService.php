...
class CountryService
{
	...
	{
		return \Cache::remember(CacheKeys::countryByName($name), Country::CASHE_TIME, function () use ($name) {
	...
	{
		$this->countries = \Cache::remember(CacheKeys::countryAllOrderBy($orderBy), Country::CASHE_TIME, function () use ($orderBy) {
	...
	{
		return \Cache::remember(CacheKeys::countryById($id), Country::CASHE_TIME, function () use ($id) {
			return Country::select('*')
				->where('id', $id)
				->firstOrFail();
		});
